...
export function AuroraConsole({
  ...
  const [lines, setLines] = useState<Entry[]>([
    { type: "out", text: "Microsoft Windows [AuroraOS]" },
    { type: "out", text: "(c) Aurora Corporation. All rights reserved." },
  ]);
  ...
  function handle(cmdline: string) {
    ...
    switch ((name || "").toLowerCase()) {
      case "help":
        write(
          "Commands: help, echo, time, date, ver, cls, whoami, cd, dir, clear, exit",
        );
        break;
      case "echo":
        write(arg);
      ...
      case "dir":
        write(
          "<DIR> apps  <DIR> users  console.exe  browser.exe  settings.exe",
        );
        break;
      case "exit":
        onOpenChange(false);
